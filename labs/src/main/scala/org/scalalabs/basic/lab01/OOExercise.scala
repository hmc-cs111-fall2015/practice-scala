...
class Euro(val euro: Int, val cents: Int = 0)
  extends Currency("EUR") with Ordered[Euro] {

  val inCents = 100 * euro + cents

  def +(that: Euro) = Euro.fromCents(this.inCents + that.inCents)

  def *(i: Int) = Euro.fromCents(this.inCents * i)

  override def toString() = {
    if (this.cents == 0)
      s"${this.symbol}: ${euro},--"
    else
      f"${this.symbol}: ${euro},${cents}%02d"
  }

  def compare(that: Euro) = this.inCents - that.inCents
}

// This provides integer * Euro functionality
object IntExtend {
  implicit class IntWithCurrency(n: Int) {
    def *(that: Euro): Euro = that * n
  }
}
