 ...
 */
class Euro(euro1: Int, cents1: Int = 0) extends Currency("EUR") {

  val inCents = euro1 + cents1
  ...
  override def toString() = {
    if (this.cents == 0)
      s"${this.symbol}: ${euro}%d,--"
    else
      s"${this.symbol}: ${euro}%d,${cents}%d"
  }
}
...
abstract class Currency(symbol1: String) {
  def symbol = symbol1
}