...
object ListManipulationExercise01 {
   ...
   */
  def sumOfList(l: List[Int]): Int = l.foldLeft(0)( (z, i) => z+i)
   ...
   *  - ... etc
   */
  def lastElementInList[T](l: List[T]): T = l.foldLeft(l.head)( (z, i) => i)
   ...
   */
  def elementExists[T](l: List[T], e: T): Boolean = l.foldLeft(false)((x,i) => if (x)
    {x} else {if (i == e) true else false}) 
   ...
   */
  def oddElements(iList: List[Int]): List[Int] = iList.foldLeft(List[Int]())((x,i) => if (i%2 == 1) x:+i else x)
